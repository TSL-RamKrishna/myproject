...
class extract_genomic_sequence():
    ...
    def __init__(self, fasta, gff):
        ...
        self.fasta = fasta
        self.gff = gff
    def read_gene_coordinates_from_gff(self, seqfeature = 'gene'):
        ...
        for record in SeqIO.parse(fastaseq, 'fasta'):
            ...
            if recordid in self.gene_data.keys():
                ...
                for rec in self.gene_data[recordid].keys():
                    ...
                    print(">" + rec)
                    print(recordseq[start-1:end])


if __name__ == '__main__':
    '''
    pass assembly fasta and gff files as positional arguments
    e.g. extract_genomic_sequence.py A.chromosome.fasta A.gff
    '''
    a = extract_genomic_sequence(sys.argv[1], sys.argv[2])
